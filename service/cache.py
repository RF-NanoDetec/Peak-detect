...
def get_peak_detection_result(
    result_id: str, params: Dict[str, Any]
    ...
    if cache_key in _peak_detection_cache:
        ...
        logger.debug(
            f"Peak detection cache HIT (key={cache_key}, hits={_peak_detection_cache_hits}, misses={_peak_detection_cache_misses})"
        )
        return _peak_detection_cache[cache_key]
    else:
        _peak_detection_cache_misses += 1
        logger.debug(
            f"Peak detection cache MISS (key={cache_key}, hits={_peak_detection_cache_hits}, misses={_peak_detection_cache_misses})"
        )
        return None


def cache_peak_detection_result(
    result_id: str,
    params: Dict[str, Any],
    peaks: np.ndarray,
    ...
    logger.debug(
        f"Peak detection result cached (key={cache_key}, size={len(_peak_detection_cache)})"
    )


def get_peak_inspection_image(
    result_id: str, filtered_result_id: str, offset: int, params: Dict[str, Any]
) -> Optional[str]:
    """
    ...
    if cache_key in _peak_inspection_cache:
        ...
        logger.debug(
            f"Peak inspection cache HIT (key={cache_key}, hits={_peak_inspection_cache_hits}, misses={_peak_inspection_cache_misses})"
        )
        return _peak_inspection_cache[cache_key]
    else:
        _peak_inspection_cache_misses += 1
        logger.debug(
            f"Peak inspection cache MISS (key={cache_key}, hits={_peak_inspection_cache_hits}, misses={_peak_inspection_cache_misses})"
        )
        return None


def cache_peak_inspection_image(
    result_id: str,
    filtered_result_id: str,
    offset: int,
    params: Dict[str, Any],
    image: str,
) -> None:
    ...
    logger.debug(
        f"Peak inspection image cached (key={cache_key}, size={len(_peak_inspection_cache)})"
    )
...
def get_cache_stats() -> Dict[str, Any]:
    ...
    return {
        "peak_detection": {
            ...
            "hit_rate": _peak_detection_cache_hits
            / max(1, _peak_detection_cache_hits + _peak_detection_cache_misses),
        },
        "peak_inspection": {
            ...
            "hit_rate": _peak_inspection_cache_hits
            / max(1, _peak_inspection_cache_hits + _peak_inspection_cache_misses),
        },
        "auto_cutoff": {
