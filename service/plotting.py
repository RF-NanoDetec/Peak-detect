...
def generate_preprocessing_plot(time_data, amplitude_data, filtered_amplitude=None):
    ...
    try:
        ...
        if len(time_data) > MAX_PLOT_POINTS:
            logger.info(
                f"Decimating {original_len} points to {MAX_PLOT_POINTS} for plotting"
            )
            t_plot_sec, amp_decimated = decimate_min_max(
                time_data, amplitude_data, MAX_PLOT_POINTS
            )
            t_plot = t_plot_sec / 60  # Convert to minutes

            if filtered_amplitude is not None:
                _, filtered_decimated = decimate_min_max(
                    time_data, filtered_amplitude, MAX_PLOT_POINTS
                )
            else:
        ...
        ax.set_title(
            "Signal Comparison" if filtered_decimated is not None else "Signal Preview",
            color="white",
            fontsize=12,
        )
        ...
        logger.info(
            f"Generated preprocessing plot: {original_len} points (decimated to {len(t_plot)} for rendering)"
        )
        return img_base64
...
def generate_detection_plot_with_peaks(
    time_data, amplitude_data, peak_times, peak_amplitudes
):
    """
    ...
    try:
        ...
        if len(time_data) > MAX_PLOT_POINTS:
            logger.info(
                f"Decimating {original_len} points to {MAX_PLOT_POINTS} for detection plot"
            )
            t_plot_sec, amp_decimated = decimate_min_max(
                time_data, amplitude_data, MAX_PLOT_POINTS
            )
        ...
        ax.set_ylabel("Counts", color="white", fontsize=10)
        ax.set_title(
            f"Peak Detection ({len(peak_times):,} peaks)", color="white", fontsize=12
        )
        ax.grid(True, linestyle="--", alpha=0.3, color="gray")
        ax.tick_params(colors="white", labelsize=9)
        ...
        logger.info(
            f"Generated detection plot: {original_len} points, {len(peak_times)} peaks"
        )
        return img_base64
...
def generate_peak_regions_plot(
    time_data, amplitude_data, filtered_amplitude, peaks, peak_properties, offset=0
):
    """
    ...
    try:
        ...
        logger.info(
            f"Generated peak inspection plot: showing peaks {offset}-{offset + 10} of {total_peaks}"
        )
        return img_base64
...
def generate_peak_histograms(peak_amplitudes, peak_widths_ms, peak_intervals_ms):
    ...
    try:
        ...
        ax3.set_xlabel(
            "Distance Between Peaks (ms, log scale)", color="white", fontsize=10
        )
