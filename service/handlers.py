...
from core.peak_analysis_utils import estimate_peak_widths, find_peaks_with_window
...
from core.timing import create_tracker
...
def open_files(
    payload: LoadFilesRequest, store: InMemoryStore = Depends(get_store)
) -> LoadFilesResponse:
...
async def upload_files(
    ...
    try:
        ...
        # Log summary
        logger.info(
            f"Data loading (upload) completed in {tracker.get_total_time():.3f}s"
        )
        tracker.log_summary(logger)
...
def get_result_binary(
    ...
    headers = {
        ...
        "X-Result-Time-Resolution": str(
            result.get("meta", {}).get("time_resolution", "")
        ),
    }

    return StreamingResponse(
        iter_chunks(), media_type="application/octet-stream", headers=headers
    )
...
def preprocess_run(
    ...
    def work(
        progress_callback, *, base_result, params: Params, original_result_id: str
    ):
        import logging
        ...
        logger.info(
            f"PREPROCESSING: Starting with {len(time_data)} data points (FULL ARRAY)"
        )

        progress_callback(
            {"progress": 1, "status": "running", "message": "Starting preprocessing"}
        )
        progress_callback(
            {"progress": 10, "status": "running", "message": "Loading data"}
        )
        ...
        progress_callback(
            {
                "progress": 30,
                "status": "running",
                "message": f"Applying {filter_type} filter",
            }
        )
        ...
        if filter_type == "none":
            ...
            progress_callback(
                {"progress": 50, "status": "running", "message": "No filter applied"}
            )
            ...
            with tracker.phase("butterworth_filter"):
                ...
                with tracker.phase("butterworth_design"):
                    b, a = scipy_signal.butter(
                        butter_order, normalized_cutoff, btype="low"
                    )
            ...
            with tracker.phase("savgol_filter"):
                ...
                if window_length > len(amplitude_data):
                    window_length = (
                        len(amplitude_data)
                        if len(amplitude_data) % 2 == 1
                        else len(amplitude_data) - 1
                    )
                ...
                filtered_amplitude = scipy_signal.savgol_filter(
                    amplitude_data, window_length, polyorder
                )
        ...
        progress_callback(
            {"progress": 70, "status": "running", "message": "Saving filtered data"}
        )
        ...
        progress_callback(
            {"progress": 90, "status": "running", "message": "Finalizing"}
        )
    ...
    task_id = tm.submit(
        ...
        original_result_id=payload.resultId,
    )
...
def detect_run(
    payload: DetectPeaksRequest, store: InMemoryStore = Depends(get_store)
) -> Dict[str, Any]:
    import logging
    ...
    with tracker.phase("load_data"):
        ...
        if filtered_result_id:
            ...
            if filtered_result:
                logger.info(
                    f"PEAK DETECTION: Using FILTERED data ({len(filtered_result['amplitude'])} points)"
                )
            else:
                logger.warning(
                    "PEAK DETECTION: filteredResultId provided but result not found"
                )
        else:
            logger.info("PEAK DETECTION: Using RAW data")

    # Log array size to confirm full-data processing
    logger.info(
        f"PEAK DETECTION: Starting with {len(base['time'])} data points (FULL ARRAY)"
    )
    logger.info(
    ...
    logger.info(
        f"PEAK DETECTION: Signal min={float(np.min(signal)):.2f}, max={float(np.max(signal)):.2f}, mean={float(np.mean(signal)):.2f}"
    )
    ...
    if filtered_result:
        ...
        with tracker.phase("auto_threshold_calc"):
            ...
            auto_threshold_filtered = calculate_auto_threshold(
                signal, sigma_multiplier=5.0
            )
            threshold_ratio = (
                payload.prominence_threshold / auto_threshold_filtered
                if auto_threshold_filtered > 0
                else 1.0
            )
            logger.info("PEAK DETECTION: Using FILTERED data for detection")
            logger.info(
                f"PEAK DETECTION: Auto threshold for filtered data: {auto_threshold_filtered:.2f}, provided threshold: {payload.prominence_threshold:.2f} (ratio: {threshold_ratio:.2f}x)"
            )
            ...
            if abs(threshold_ratio - 1.0) > 0.5:  # More than 50% difference
                logger.warning(
                    f"PEAK DETECTION: WARNING - Threshold ({payload.prominence_threshold:.2f}) differs significantly from auto threshold for filtered data ({auto_threshold_filtered:.2f}). "
                    f"Consider recalculating threshold from filtered data using 'Auto Threshold' button."
                )
    else:
        logger.info("PEAK DETECTION: Using ORIGINAL data for detection")
    ...
    if result is None:
        ...
        logger.info(
            f"Peak detection completed in {tracker.get_total_time():.3f}s (no peaks found)"
        )
        tracker.log_summary(logger)
        return {
            "count": 0,
            "peaks": [],
            "stats": {"mean_height": 0.0, "mean_width": 0.0},
        }
    ...
    with tracker.phase("calculate_stats"):
        if len(peaks_list) > 0:
            ...
            peak_times = [
                float(base["time"][i]) for i in peaks_list
            ...
            logger.info(
                f"PEAK DETECTION: Found {len(peaks_list)} peaks, mean height={mean_height:.2f}, peak heights range=[{min(peak_heights):.2f}, {max(peak_heights):.2f}]"
            )
        else:
            mean_height = 0.0
            mean_width = 0.0
...
def auto_cutoff(
    payload: Dict[str, str], store: InMemoryStore = Depends(get_store)
) -> Dict[str, float]:
    """
    ...
    avg_width_sec = estimate_peak_widths(
        signal, fs, prominence_threshold=threshold, time_resolution=time_resolution
    )
    ...
    logger.info(
        f"Auto-cutoff: calculated cutoff={cutoff_hz} Hz, rounded to {cutoff_hz_int} Hz"
    )
...
def auto_threshold(
    ...
    if filtered_result_id:
        ...
        if filtered_result:
            ...
            logger.info(
                f"Auto-threshold: Using FILTERED data (filteredResultId={filtered_result_id}, {len(signal)} points)"
            )
            logger.info(
                f"Auto-threshold: Filtered signal stats - min={float(np.min(signal)):.2f}, max={float(np.max(signal)):.2f}, mean={float(np.mean(signal)):.2f}"
            )
        else:
            logger.warning(
                "Auto-threshold: filteredResultId provided but result not found; using original data"
            )
    else:
        logger.info(f"Auto-threshold: Using ORIGINAL data ({len(signal)} points)")
        logger.info(
            f"Auto-threshold: Original signal stats - min={float(np.min(signal)):.2f}, max={float(np.max(signal)):.2f}, mean={float(np.mean(signal)):.2f}"
        )
    ...
    logger.info(
        f"Auto-threshold: Calculated from {data_source} data - sigma={sigma_multiplier}, threshold={threshold:.2f}, rounded to integer: {threshold_int}"
    )
...
def generate_histograms(
    ...
    try:
        ...
        hist_data = generate_peak_histogram_data(
            peak_amplitudes, peak_widths_ms, peak_intervals_ms, config=config
        )
    ...
    try:
        ...
        def metric_hist(values, metric_key: str):
            try:
                ...
                x_scale = (
                    metric_cfg.get("xScale") or metric_cfg.get("x_scale") or "linear"
                )
                range_override = (
        ...
        logger.info(
            f"Generated double peak histograms: distance={len(distance_hist['bins'])} bins, "
        ...
        logger.error(
            f"Error calculating double peak histogram data: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Error calculating histogram data: {str(e)}"
        )
...
def process_run(
    ...
    def work(progress_callback, *, base_result, params: Params):
        ...
        return {
            "peaks": None if peaks_tuple is None else [int(i) for i in peaks_tuple[0]]
        }
    ...
    task_id = tm.submit(
        work, on_progress=on_progress, base_result=base, params=payload.params
    )
    return {"taskId": task_id}
...
def analyze_double_peaks(
    ...
    logger.info(
        f"Double peak analysis: {result['double_peak_count']} double peaks found out of {result['total_pairs']} pairs"
    )
...
def export_unified(payload: Dict[str, Any], store: InMemoryStore = Depends(get_store)):
    """
    ...
    if isinstance(double_peak_indices, list):
        double_peak_indices = [
            int(idx) for idx in double_peak_indices if isinstance(idx, (int, float))
        ]
    double_peak_thresholds = payload.get("double_peak_thresholds") or {}
    selection_source = payload.get("selection_source") or (
        "lasso" if double_peak_indices else "filters"
    )
    ...
    time_resolution = payload.get("time_resolution") or base["meta"].get(
    ...
    if needs_double_section and not double_peak_analysis:
        params_to_use = double_peak_params or {
            "min_distance": double_peak_thresholds.get("distance", [0.001, 0.100])[0]
            / 1000
            if double_peak_thresholds
            else 0.001,
            "max_distance": double_peak_thresholds.get("distance", [0.001, 0.100])[1]
            / 1000
            if double_peak_thresholds
            else 0.100,
            "min_amp_ratio": double_peak_thresholds.get("pairPromRatio", [0.1, 10.0])[
                0
            ],
            "max_amp_ratio": double_peak_thresholds.get("pairPromRatio", [0.1, 10.0])[
                1
            ],
            "min_width_ratio": double_peak_thresholds.get(
                "pairWidthRatio", [0.1, 10.0]
            )[0],
            "max_width_ratio": double_peak_thresholds.get(
                "pairWidthRatio", [0.1, 10.0]
            )[1],
        }
    ...
    if needs_double_section:
        ...
        if double_peak_thresholds:
            ...
            if prom_over_amp and len(prom_over_amp) == 2:
                double_metadata_items.append(
                    ("Prominence/Amplitude Min", prom_over_amp[0])
                )
                double_metadata_items.append(
                    ("Prominence/Amplitude Max", prom_over_amp[1])
                )

        if double_peak_analysis:
            double_metadata_items.append(
                ("Total Pairs", double_peak_analysis.get("total_pairs", 0))
            )
            double_metadata_items.append(
                ("Double Peak Count", double_peak_analysis.get("double_peak_count", 0))
            )

        selected_count = (
            len(double_df)
            if not double_df.empty
            else (len(double_peak_indices) if double_peak_indices else 0)
        )
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            if include_metadata and (single_metadata_items or double_metadata_items):
                ...
                meta_df = pd.DataFrame(
                    meta_rows, columns=["Section", "Parameter", "Value"]
                )
