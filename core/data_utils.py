...
def timestamps_to_seconds(timestamp_str):
    ...
    try:
        ...
        logger.error(
            f"Error converting timestamp to seconds: {str(e)}\n{traceback.format_exc()}"
        )
        return 0.0
...
def convert_width_ms_to_samples(width_ms, sampling_rate):
    ...
    try:
        ...
        logger.error(
            f"Error converting width from ms to samples: {e}\n{traceback.format_exc()}"
        )
...
def reset_application_state(app):
    ...
    try:
        ...
        if hasattr(app, "results_summary"):
            # Clear results summary if it exists
            try:
                app.results_summary.set("")
            except Exception:
                pass

        # Update the preview label
        app.preview_label.config(
            text="", foreground=app.theme_manager.get_color("text")
        )
        ...
        logger.error(
            f"Error resetting application state: {str(e)}\n{traceback.format_exc()}"
        )
...
def reset_application_state_with_ui(app):
    ...
    try:
        ...
        # Log the error (UI error dialog not available in core module)
        logger.error(f"Failed to reset application state: {str(e)}")
