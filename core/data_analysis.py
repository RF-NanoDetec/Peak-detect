...
from core.peak_detection import (
    calculate_auto_threshold as peak_detection_auto_threshold,
)
...
def calculate_peak_areas(
    ...
    try:
        ...
        logger.error(
            f"Error calculating peak areas: {str(e)}\n{traceback.format_exc()}"
        )
        return None
...
def calculate_peak_intervals(t_value, peaks_indices):
    ...
    try:
        ...
        logger.error(
            f"Error calculating peak intervals: {str(e)}\n{traceback.format_exc()}"
        )
        return []
...
def analyze_time_resolved(app):
    ...
    try:
        ...
        time_res = (
            app.time_resolution.get()
            if hasattr(app.time_resolution, "get")
            else app.time_resolution
        )
        if time_res <= 0:
        ...
        logger.error(
            f"Error in analyze_time_resolved: {str(e)}\n{traceback.format_exc()}"
        )
        return None
...
def analyze_time_resolved_pure(
    ...
    try:
        ...
        logger.error(
            f"Error in analyze_time_resolved_pure: {str(e)}\n{traceback.format_exc()}"
        )
        return None
...
def calculate_auto_threshold(signal, percentile=95, sigma_multiplier=None):
    ...
    try:
        ...
        logger.error(
            f"Error calculating auto threshold: {str(e)}\n{traceback.format_exc()}"
        )
