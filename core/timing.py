...
class TimingTracker:
    ...
    def get_phase_total_times(self) -> Dict[str, float]:
        ...
        with self._lock:
            timings_copy = {phase: list(times) for phase, times in self.timings.items()}
        return {phase: sum(times) for phase, times in timings_copy.items()}
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if total_time > 0:
            percentages = {
                phase: (time / total_time) * 100 for phase, time in phase_times.items()
            }
        ...
        summary = {
            ...
            "slowest_phase": {
                ...
                "percentage": percentages.get(slowest_phase, 0.0)
                if slowest_phase
                else 0.0,
            },
            "metadata": {},
    ...
    def get_formatted_summary(self) -> str:
        ...
        sorted_phases = sorted(
            summary["phases"].items(), key=lambda x: x[1]["total_seconds"], reverse=True
        )
