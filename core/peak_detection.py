...
class PeakDetector:
    ...
    def detect_peaks(self, signal, time_values, height_lim, distance, prominence_ratio, rel_height=0.5, width_range=None, time_resolution=1e-4):
        ...
        Parameters:
            ...
            prominence_ratio (float): Threshold for the ratio of prominence to peak height.
                ...
                Lower values (e.g., 0.5) are more permissive.
...
@profile_function
def detect_peaks(signal, time_resolution, height_lim, distance, prominence_ratio, rel_height=0.5, width_range=None):
    """
    Detect peaks in the provided signal data and calculate their areas.

    This function identifies peaks meeting specified criteria and calculates
    their areas using the trapezoidal rule based on peak width properties.

    Parameters:
        signal (numpy.ndarray): The signal data to analyze.
        time_resolution (float): Time interval between data points in seconds.
        height_lim (float): Minimum height threshold for peaks (prominence).
        distance (int): Minimum number of samples between peaks.
        prominence_ratio (float): Minimum prominence/height ratio for filtering subpeaks.
        rel_height (float, optional): Relative height for width measurement. Defaults to 0.5.
        width_range (tuple, optional): Tuple of (min_width, max_width) in milliseconds to filter peaks.
            If None, no width filtering is applied.

    Returns:
        tuple: (peaks, properties) where:
            - peaks is a numpy array of peak indices.
            - properties is a dict containing peak properties (prominences, widths, areas, etc.).
              Returns (None, None) if no peaks are detected or an error occurs.

    Raises:
        ValueError: If input parameters are invalid.
    """
    if signal is None or len(signal) == 0:
        logger.warning("detect_peaks called with empty signal.")
        return None, None
    ...
    try:
        rate = time_resolution
        sampling_rate = 1.0 / rate

        width_p = None
        if width_range:
            try:
                min_w_ms, max_w_ms = map(float, width_range)
                width_p = [int(min_w_ms * sampling_rate / 1000), int(max_w_ms * sampling_rate / 1000)]
                logger.debug(f"Width range (ms): {width_range} -> Width range (samples): {width_p}")
            except (ValueError, TypeError, IndexError) as e:
                logger.error(f"Invalid width_range format: {width_range}. Error: {e}")
                width_p = None # Ignore invalid width range

        # Find peaks using the utility function that handles prominence ratio filtering
        peaks, properties = find_peaks_with_window(
            signal,
            width=width_p, # Pass converted width in samples
            prominence=height_lim,
            distance=distance,
            rel_height=rel_height,
            prominence_ratio=prominence_ratio
        ...
        if len(peaks) == 0:
            logger.info("No peaks detected with current parameters.")
            return None, None

        # Calculate peak areas using trapezoidal rule based on width properties
        areas = []
        if "left_ips" in properties and "right_ips" in properties:
            for i, peak_idx in enumerate(peaks):
                ...
                if left_idx < right_idx:
                    # Calculate area using signal segment defined by width boundaries
                    area = np.trapz(signal[left_idx:right_idx])
                    areas.append(area)
                else:
                    logger.warning(f"Invalid indices for peak {peak_idx} area calculation: [{left_idx}:{right_idx}]")
                    areas.append(0.0) # Append 0 if indices are invalid
            properties['areas'] = np.array(areas)
        else:
            logger.warning("Peak width properties (left_ips, right_ips) not found. Cannot calculate areas.")
            properties['areas'] = np.zeros(len(peaks)) # Assign zero areas if widths are missing

        logger.info(f"Detected {len(peaks)} peaks.")
        return peaks, properties

    except Exception as e:
        logger.error(f"Error during peak detection: {str(e)}\n{traceback.format_exc()}")
