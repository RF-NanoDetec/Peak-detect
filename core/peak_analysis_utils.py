...
from scipy.signal import butter, filtfilt, find_peaks, peak_widths, savgol_filter

# Import profiling utilities from the performance module
from core.performance import profile_function, profiling_log
...
def apply_butterworth_filter(order, Wn, btype, fs, x):
    ...
    profiling_log(
        f"Filtered signal: {x_f[:10]}..."
    )  # Printing the first 10 values for brevity
    return x_f
...
def timestamps_array_to_seconds(timestamps, start_time):
    ...
    try:
        ...
        raise ValueError(
            f"Error converting timestamps: {e}\n"
...
def find_peaks_with_window(
    signal, width, prominence, distance, rel_height, prominence_ratio
):
    """
    ...
    if len(peaks) > 0:
        ...
        properties["width_heights"] = width_results[
            1
    ...
    peaks, properties = filter_subpeaks(
        signal, peaks, properties, prominence_ratio_threshold=prominence_ratio
    )
...
def estimate_peak_widths(
    ...
    **kwargs,
    ...
    _logger = logging.getLogger(__name__)
    _logger.debug(
        "---- Starting estimate_peak_widths (using fastest 10% of peaks) ----"
    )
    ...
    peaks, _ = find_peaks(
        working_signal, width=[1, 20000], prominence=prominence_threshold, distance=500
    )
    ...
    _logger.debug(
        f"Width range: min={np.min(widths):.2f}, max={np.max(widths):.2f} samples"
    )
    _logger.debug(
        f"Fastest 10% width range: min={np.min(fastest_widths):.2f}, max={np.max(fastest_widths):.2f} samples"
    )
...
def adjust_lowpass_cutoff(
    ...
    profiling_log("DEBUG: Input parameters:")
    profiling_log(f"- fs: {fs}")
    if filter_type == "butterworth":
        profiling_log(f"- manual_cutoff_hz (input): {manual_cutoff_hz}")
        profiling_log(
            f"- prominence_threshold_butter_cutoff_calc (for auto): {prominence_threshold_butter_cutoff_calc}"
        )
        profiling_log(
            f"- normalization_factor_butter (for auto): {normalization_factor_butter}"
        )
        ...
        profiling_log(
            f"- prominence_threshold_savgol_window_est: {prominence_threshold_savgol_window_est}"
        )
    ...
    if filter_type == "butterworth":
        profiling_log("DEBUG: Butterworth mode selected.")
        final_cutoff_hz = 0.0

        if manual_cutoff_hz > 0:
            profiling_log(
                f"DEBUG: Using provided manual_cutoff_hz: {manual_cutoff_hz} Hz"
            )
            ...
            profiling_log(
                "DEBUG: manual_cutoff_hz is 0 or not provided. Auto-calculating Butterworth cutoff."
            )
            avg_width_sec = estimate_peak_widths(
                signal, fs, prominence_threshold_butter_cutoff_calc, time_resolution
            )
            profiling_log(
            ...
            if avg_width_sec <= 1e-9:
                profiling_log(
                    f"Warning: avg_width_sec ({avg_width_sec:.2e}) is very small or zero. "
                )
                base_cutoff_hz = fs / 4.0
                profiling_log(
                    f"Using a fallback base_cutoff_hz: {base_cutoff_hz:.2f} Hz due to small avg_width_sec."
                )
            else:
                base_cutoff_hz = 1.0 / avg_width_sec
            profiling_log(
                f"DEBUG: Base cutoff frequency (1/avg_width_sec): {base_cutoff_hz:.2f} Hz"
            )

            normalized_cutoff = base_cutoff_hz * float(normalization_factor_butter)
            profiling_log(
                f"DEBUG: Normalized cutoff (base * norm_factor): {normalized_cutoff:.2f} Hz"
            )
            final_cutoff_hz = normalized_cutoff
            applied_filter_params["estimated_avg_width_sec"] = avg_width_sec
            applied_filter_params["prominence_for_cutoff_calc"] = (
                prominence_threshold_butter_cutoff_calc
            )
        ...
        profiling_log(
            f"DEBUG: Final Butterworth cutoff to be applied: {final_cutoff_hz:.2f} Hz"
        )

        filtered_signal = apply_butterworth_filter(
            butter_order, final_cutoff_hz, "lowpass", fs, signal
        )
        profiling_log(
            f"DEBUG: Butterworth filter applied with order={butter_order}, cutoff_hz={final_cutoff_hz:.2f} Hz"
        )
        ...
        profiling_log("DEBUG: Savitzky-Golay mode selected.")
        ...
        if len(signal) < 3:  # SavGol not applicable for very short signals
            profiling_log(
                f"ERROR: Signal length ({len(signal)}) is too short for Savitzky-Golay filter. Returning unfiltered signal."
            )
            filtered_signal = signal.copy()  # Return original signal
            applied_filter_params["error"] = (
                "Signal too short for Sav-Gol filter (min length 3)."
            )
            ...
            if current_savgol_window_length is None:
                profiling_log(
                    f"DEBUG: Sav-Gol window_length not provided. Estimating using prominence: {prominence_threshold_savgol_window_est}"
                )
                s_peaks, _ = find_peaks(
                    signal, prominence=prominence_threshold_savgol_window_est
                )
                if len(s_peaks) > 0:
                    s_width_results = peak_widths(signal, s_peaks, rel_height=0.5)
                    avg_s_width_samples = np.mean(
                        s_width_results[0]
                    )  # Widths in samples
                    profiling_log(
                        f"DEBUG: Estimated avg peak width for Sav-Gol: {avg_s_width_samples:.2f} samples"
                    )

                    estimated_wl = int(
                        np.ceil(avg_s_width_samples * 1.5)
                    )  # Heuristic: 1.5x avg peak width
                    current_savgol_window_length = estimated_wl
                    applied_filter_params["estimated_window_source"] = (
                        "peak_width_heuristic"
                    )
                    applied_filter_params["estimated_avg_samples_width"] = (
                        avg_s_width_samples
                    )
                else:
                    profiling_log(
                        f"DEBUG: No peaks found for Sav-Gol window estimation (prominence {prominence_threshold_savgol_window_est}). Using fallback."
                    )
                    fallback_wl = min(
                        max(5, int(len(signal) * 0.05)), 101
                    )  # Fallback: 5% of signal, min 5, max 101
                    current_savgol_window_length = fallback_wl
                    applied_filter_params["estimated_window_source"] = (
                        "fallback_no_peaks"
                    )
                profiling_log(
                    f"DEBUG: Auto-determined Sav-Gol window_length: {current_savgol_window_length}"
                )

            # Ensure window length is odd and positive
            if current_savgol_window_length < 3:
                current_savgol_window_length = 3
            if current_savgol_window_length % 2 == 0:
                current_savgol_window_length += 1
                profiling_log(
                    f"Adjusted Sav-Gol window_length to be odd: {current_savgol_window_length}"
                )
            ...
            if current_savgol_polyorder is None:
                ...
                current_savgol_polyorder = max(
                    1, current_savgol_polyorder
                )  # Must be at least 1
                profiling_log(
                    f"DEBUG: Using default Sav-Gol polyorder: {current_savgol_polyorder}"
                )
            ...
            if current_savgol_window_length > len(signal):
                profiling_log(
                    f"Warning: Sav-Gol window_length {current_savgol_window_length} > signal length {len(signal)}. Adjusting to signal length or slightly less."
                )
                current_savgol_window_length = (
                    len(signal) if len(signal) % 2 != 0 else len(signal) - 1
                )
                if current_savgol_window_length < 3:
                    current_savgol_window_length = 3  # if signal itself is very short

            if current_savgol_polyorder >= current_savgol_window_length:
                profiling_log(
                    f"Warning: Sav-Gol polyorder {current_savgol_polyorder} >= window_length {current_savgol_window_length}. Adjusting polyorder."
                )
            ...
            profiling_log(
                f"DEBUG: Final Sav-Gol params before filtering: window_length={current_savgol_window_length}, polyorder={current_savgol_polyorder}"
            )

            if (
                current_savgol_window_length > len(signal)
                or current_savgol_window_length <= current_savgol_polyorder
                or current_savgol_window_length < 1
            ):
                profiling_log(
                    f"ERROR: Invalid Savitzky-Golay parameters for signal of length {len(signal)}: "
                    f"window_length={current_savgol_window_length}, polyorder={current_savgol_polyorder}. "
                    "Returning unfiltered signal."
                )
                filtered_signal = signal.copy()  # Return original signal
                applied_filter_params["error"] = (
                    "Invalid Sav-Gol parameters (e.g., window too large, polyorder too large/small)."
                )
                ...
                try:
                    filtered_signal = savgol_filter(
                        signal, current_savgol_window_length, current_savgol_polyorder
                    )
                    profiling_log("DEBUG: Savitzky-Golay filter applied.")
                except ValueError as e:
                    profiling_log(
                        f"ERROR applying Savitzky-Golay filter: {e}. Returning unfiltered signal."
                    )
        ...
        profiling_log(
            f"ERROR: Unsupported filter_type: {filter_type}. Choose 'butterworth' or 'savgol'. Returning unfiltered signal."
        )
        filtered_signal = signal.copy()  # Return original signal
        applied_filter_params["error"] = f"Unsupported filter_type: {filter_type}"

    profiling_log(
        f'DEBUG: Filter "{filter_type}" processing complete. Final effective params: {applied_filter_params}'
    )
...
def calculate_lowpass_cutoff(
    signal, fs, prominence_threshold, normalization_factor=1.0, time_resolution=1e-4
):
    """
    ...
    profiling_log("DEBUG: After normalization:")
...
def compute_baseline_mask(
    ...
    for i in range(num_peaks):
        ...
        if (
            ...
            and i < len(left_indices)
            and i < len(right_indices)
        ):
