...
The application opens with a dark theme by default. You can switch to light theme via **View ÔåÆ Toggle Theme**.

### Step 2: Load Data

1. Click **File ÔåÆ Open** or press `Ctrl+O`
...
- **Time Resolution**: 0.0001 s (100 ╬╝s per sample)
...
- **File ÔåÆ Export Plot**: Save the current visualization
- **File ÔåÆ Export Data ÔåÆ Peak Information**: Save peak data as CSV
...
|-----------|-------------------|-----------|
| Sampling Rate | ÔëÑ 10 kHz | Capture fast transients |
| Signal Length | ÔëÑ 1000 points | Statistical reliability |
| SNR | > 3 | Distinguish peaks from noise |
| Dynamic Range | ÔëÑ 12 bits | Avoid quantization artifacts |
...
```
Raw Data ÔåÆ Filtering ÔåÆ Peak Detection ÔåÆ Property Calculation ÔåÆ Statistical Analysis
```
...
The time resolution parameter (default: 0.0001 s = 100 ╬╝s) determines:
...
- **Grid Layout**: 10 peaks shown simultaneously (2├ù5 grid)
...
1. Time separation: `min_distance` < ╬öt < `max_distance`
2. Amplitude ratio: `min_ratio` < AÔéé/AÔéü < `max_ratio`
...
2. Click **Tools ÔåÆ Analyze Time-Resolved**
...
### Export Options
1. Current plot: **File ÔåÆ Export Plot** (Ctrl+E)
