...
- [x] Duplicate action bars: keep only menu bar + one primary toolbar; remove functional bar under preview and any duplicate theme toggles/actions
- [x] Tooltip contrast: ensure high-contrast foreground/background for dark and light themes; unify tooltip styling via tokens
...
- [x] Preprocessing default controls: on first load, show only Butterworth controls; hide Savitzky–Golay until selected
- [x] Theme ripple: on theme toggle, refresh canvases, sections, and tooltips to adopt new colors immediately
