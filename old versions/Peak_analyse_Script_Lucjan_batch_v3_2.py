...
class Application(tk.Tk):
    ...
    def browse_file(self):
        ...
        print(f"Current file mode: {self.file_mode.get()}")  # Debug print
        ...
        try:
            ...
            if files:
                # Sort using Tcl dictionary sorting
                files = list(Tcl().call('lsort', '-dict', files))
                print(f"Sorted files: {files}")  # Debug print
            else:
            ...
            if self.file_mode.get() == "batch":
                timestamp_str = self.batch_timestamps.get().strip()
                if timestamp_str:
                    timestamps = [t.strip() for t in timestamp_str.split(',')]
                    if len(timestamps) != len(files):
                        raise ValueError(f"Number of timestamps ({len(timestamps)}) "
                                      f"doesn't match number of files ({len(files)})")
            
            print(f"Processing {len(files)} files...")  # Debug print
            ...
            for i, file in enumerate(files):
                print(f"Loading file {i+1}/{len(files)}: {file}")  # Debug print
