...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # Add File Order display in protocol section
        self.protocol_files_label = ttk.Label(control_frame, text="Loaded Files")
        self.protocol_files_label.grid(row=38, column=0, padx=5, pady=5, sticky="w")  # Adjust row number as needed
        
        # Use Text widget instead of Entry for better multiline display
        self.protocol_files_text = ScrolledText(
            control_frame,
            height=5,  # Adjust height as needed
            width=40,  # Adjust width as needed
            wrap=tk.WORD
        )
        self.protocol_files_text.grid(row=38, column=1, padx=5, pady=5, sticky="nsew")
    ...
    def browse_file(self):
        ...
        if files:
            try:
                ...
                # Store the file names and their order
                self.loaded_files = [os.path.basename(f) for f in files]
                
                # Create file order text for protocol
                file_order_text = "\n".join([f"{i+1}. {fname}" for i, fname in enumerate(self.loaded_files)])
                self.protocol_notes.set(f"Loaded files in order:\n{file_order_text}")
                ...
                # Update preview with data info and file order
                preview_text = (
            ...
            self.preview_label.config(text="No file selected", foreground="red")
    ...
    def plot_data(self):
        ...
        try:
            ...
            if not tab_exists:
                ...
                new_tab = self.tab_control.select()
    ...
    def reset_data_zoom(self):
        """Reset zoom level for data plot."""
        try:
            axes = self.data_figure.get_axes()
            for ax, xlim, ylim in zip(axes, self.data_original_xlims, self.data_original_ylims):
                ax.set_xlim(xlim)
                ax.set_ylim(ylim)
            self.data_canvas.draw()
        except Exception as e:
            self.show_error("Error resetting data view", e)
