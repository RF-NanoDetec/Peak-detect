...
def plot_single_peak_analysis():
    ...
    peaks, props = detector.detect_peaks(
        filtered_signal, t, height_lim=1.0, distance=50, prominence_ratio=0.5
    )
    ...
    ax2.plot(
        t[peaks], filtered_signal[peaks], "rx", markersize=10, label="Detected Peaks"
    )
    ...
    for i, peak in enumerate(peaks):
        ...
        left_edge = props["left_ips"][i] * (t[1] - t[0])
        right_edge = props["right_ips"][i] * (t[1] - t[0])
        ax2.hlines(h, left_edge, right_edge, color="g", linestyle="--", alpha=0.8)
        if i == 0:
...
def plot_dead_time_correction():
    ...
    corrected_counts, info = apply_dead_time_correction(
        measured_counts, time_res, dead_time_ns=43.0
    )
    ...
    ax1.axhline(
        saturation_limit, color="k", linestyle=":", label="Theoretical Saturation Limit"
    )
...
def plot_baseline_noise():
    ...
    ax.axhline(
        median + 3 * sigma_mad, color="r", linestyle="--", label="+3$\sigma_{MAD}$"
    )
    ax.axhline(
        median - 3 * sigma_mad, color="r", linestyle="--", label="-3$\sigma_{MAD}$"
    )
